...
ProductSchema.post("save", async function (doc) {
  const ProductArea = (await import("../models/mappings/ProductArea.js")).default as any;
  ...
  if (doc.is_global) {
    ...
    if (existingMappings.length === 0) {
      const productAreaMappings = areas.map((area: { _id: Types.ObjectId; name_hindi: string }) => ({
        product: doc._id,
        area: area._id,
