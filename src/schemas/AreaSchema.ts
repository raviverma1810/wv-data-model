...
import { CategoryArea, Category, ProductArea, Product } from "../models/index";
...
AreaSchema.post("save", async function (doc) {
  ...
  const existingCategoryMappings = await CategoryArea.find({
    category: {
      $in: globalCategories.map(
        (category: { _id: Types.ObjectId }) => category._id,
      ),
    },
    area: doc._id,
  });
  ...
  if (existingCategoryMappings.length === 0) {
    const categoryAreaMappings = globalCategories.map(
      (category: { _id: Types.ObjectId; name_hindi: string }) => ({
  ...
  const existingProductMappings = await ProductArea.find({
    product: {
      $in: globalProducts.map(
        (product: { _id: Types.ObjectId }) => product._id,
      ),
    },
    area: doc._id,
  });
